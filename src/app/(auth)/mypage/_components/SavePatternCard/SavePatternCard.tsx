...
export function SavePatternCard({ data }: SavePatternCardProps) {
  ...
  return (
    <StatCard title="링크 저장 패턴" date={data.period} description={data.text}>
      <div className="flex h-full items-end justify-center gap-4 pt-10">
        {data.counts.map((item) => {
          ...
          return (
            <div key={item.day} className="flex flex-col items-center gap-8">
              <div className="relative h-120 w-24">
                <div className="rounded-8 bg-blue-light absolute inset-0" />
                <div
                  className={`rounded-8 absolute right-0 bottom-0 left-0 transition-all duration-300 ${
              ...
              <div
                className={`rounded-8 bg-gray-box flex w-24 flex-col items-center justify-center px-2 py-4 ${isPeak ? 'border border-[#565AC8]' : 'border-none'}`}
              >
